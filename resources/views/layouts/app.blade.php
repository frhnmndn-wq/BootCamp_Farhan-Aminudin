            ...
            <div class="flex items-center justify-between h-16 sm:h-20">
                ...
                <div class="flex items-center space-x-2 sm:space-x-3">
                    ...
                    <div class="flex flex-col">
                        <span class="font-extrabold text-lg sm:text-xl tracking-tight bg-gradient-to-r from-white via-blue-100 to-blue-300 bg-clip-text text-transparent">MicroSite</span>
                ...
                <div class="flex items-center space-x-2 sm:space-x-4">
                    ...
                    <a href="{{ route('admin.dashboard') }}" class="text-blue-200 hover:text-white hover:bg-white/10 transition-all duration-200 p-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl text-sm font-semibold flex items-center gap-2">
                        <i data-lucide="bar-chart-3" class="w-5 h-5 sm:w-4 sm:h-4"></i>
                        <span class="hidden md:inline">Dashboard</span>
                    </a>

                    <!-- MENU MANAGE LINKS (Navigasi ke CRUD Link) -->
                    <a href="{{ route('admin.links.index') }}" class="text-blue-200 hover:text-white hover:bg-white/10 transition-all duration-200 p-2 sm:px-4 sm:py-2.5 rounded-lg sm:rounded-xl text-sm font-semibold flex items-center gap-2">
                        <i data-lucide="list" class="w-5 h-5 sm:w-4 sm:h-4"></i>
                        <span class="hidden md:inline">Manage Links</span>
                    </a>

                    <!-- Preview Button (Membuka Halaman Publik di Tab Baru) -->
                    <a href="{{ route('public.index') }}" target="_blank" class="bg-blue-400 hover:bg-blue-300 text-slate-950 font-bold px-3 py-2 sm:px-5 sm:py-2.5 rounded-lg sm:rounded-xl text-xs sm:text-sm transition-all duration-200 flex items-center gap-1.5 sm:gap-2 shadow-lg shadow-blue-500/20 hover:shadow-blue-500/40 border border-blue-300">
...
<form action="{{ route('logout') }}" method="POST" class="m-0">
    ...
    <main class="max-w-7xl mx-auto py-6 sm:py-10 px-4 sm:px-6 lg:px-8 flex-grow w-full">
        ...
        @if(session('success'))
            <div class="mb-6 p-4 sm:p-5 bg-emerald-200 text-emerald-950 font-extrabold rounded-2xl border-2 border-slate-900 shadow-[4px_4px_0px_0px_#0f172a] flex items-center justify-between gap-3">
                <div class="flex items-center gap-3">
                    <i data-lucide="check-circle-2" class="w-6 h-6 text-emerald-800 flex-shrink-0"></i>
    ...
    <footer class="bg-white border-t border-slate-200 text-center py-6 px-4 text-xs font-medium text-slate-500 mt-auto">
        &copy; {{ date('Y') }} &bull; (Microsite) Application
    </footer>
